...
check_directory_structure() {
  ...
  # Verify .pmf directory
  if [[ ! -d "$extract_dir/.pmf" ]]; then
    print_fail "Missing .pmf directory"
    return 1
  fi
  ...
  for dir in "${required_dirs[@]}"; do
    if [[ ! -d "$extract_dir/.pmf/$dir" ]]; then
      print_fail "Missing .pmf/$dir directory"
      ((errors++))
    fi
  done
...
check_constitution_version() {
  ...
  if [[ ! -f "$extract_dir/.pmf/memory/constitution.md" ]]; then
    print_fail "Constitution file not found"
    return 1
  fi

  # Check for PMF-Kit constitution header
  if ! grep -q "PMF-Kit Constitution" "$extract_dir/.pmf/memory/constitution.md"; then
...
check_script_consistency() {
  ...
  if [[ "$script_type" == "sh" ]]; then
    if [[ ! -d "$extract_dir/.pmf/scripts/bash" ]]; then
      print_fail "Script type is 'sh' but no bash/ scripts directory found"
      return 1
    fi
    if [[ -d "$extract_dir/.pmf/scripts/powershell" ]]; then
