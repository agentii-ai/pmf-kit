...
function Rewrite-Paths {
    ...
    $Content = $Content -replace '(/?)\bmemory/', '.pmf/memory/'
    $Content = $Content -replace '(/?)\bscripts/', '.pmf/scripts/'
    $Content = $Content -replace '(/?)\btemplates/', '.pmf/templates/'
    return $Content
}
...
function Generate-Commands {
    ...
    foreach ($template in $templates) {
        ...
        $outputFile = Join-Path $OutputDir "pmfkit.$name.$Extension"
...
function Generate-CopilotPrompts {
    ...
    $agentFiles = Get-ChildItem -Path "$AgentsDir/pmfkit.*.agent.md" -File -ErrorAction SilentlyContinue
...
function Build-Variant {
    ...
    $specDir = Join-Path $baseDir ".pmf"
    New-Item -ItemType Directory -Path $specDir -Force | Out-Null
    
    # Copy memory directory
    if (Test-Path "memory") {
        Copy-Item -Path "memory" -Destination $specDir -Recurse -Force
        Write-Host "Copied memory -> .pmf"
    }
    ...
    if (Test-Path "templates") {
        ...
        Write-Host "Copied templates -> .pmf/templates"
    }
