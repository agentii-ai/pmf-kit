...
rewrite_paths() {
  sed -E \
    -e 's@(^|[[:space:]]|`)/memory/@\1.pmf/memory/@g' \
    -e 's@(^|[[:space:]]|`)memory/@\1.pmf/memory/@g' \
    -e 's@(^|[[:space:]]|`)/scripts/@\1.pmf/scripts/@g' \
    -e 's@(^|[[:space:]]|`)scripts/@\1.pmf/scripts/@g' \
    -e 's@(^|[[:space:]]|`)/templates/@\1.pmf/templates/@g' \
    -e 's@(^|[[:space:]]|`)templates/@\1.pmf/templates/@g' \
    -e 's@\.pmf/\.pmf/@.pmf/@g'
}
...
generate_commands() {
  ...
  for template in .pmf/templates/commands/*.md; do
    ...
    # Also inject agent: pmfkit.{name} into frontmatter
    body=$(printf '%s\n' "$body" | awk -v name="$name" '
      /^---$/ {
        print
        if (++dash_count == 1) {
          in_frontmatter=1
          print "agent: pmfkit." name
        } else {
          in_frontmatter=0
        }
        next
    ...
    case $ext in
      toml)
        body=$(printf '%s\n' "$body" | sed 's/\\/\\\\/g')
        { echo "description = \"$description\""; echo; echo "prompt = \"\"\""; echo "$body"; echo "\"\"\""; } > "$output_dir/pmfkit.$name.$ext" ;;
      md)
        echo "$body" > "$output_dir/pmfkit.$name.$ext" ;;
      agent.md)
        echo "$body" > "$output_dir/pmfkit.$name.$ext" ;;
    esac
  done
...
generate_copilot_prompts() {
  ...
  for agent_file in "$agents_dir"/pmfkit.*.agent.md; do
...
build_variant() {
  ...
  [[ -d .pmf/templates ]] && { mkdir -p "$SPEC_DIR/templates"; cp -r .pmf/templates/* "$SPEC_DIR/templates/" 2>/dev/null || true; rm -rf "$SPEC_DIR/templates/commands" "$SPEC_DIR/templates/vscode-settings.json" 2>/dev/null || true; echo "Copied .pmf/templates -> .pmf/templates"; }
