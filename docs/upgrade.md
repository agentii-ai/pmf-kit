...
- ✅ **Script files** (`.pmf/scripts/`)
- ✅ **Template files** (`.pmf/templates/`)
- ✅ **Shared memory files** (`.pmf/memory/`) - **⚠️ See warnings below**
...
**Known issue:** `specify init --here --force` currently overwrites `.pmf/memory/constitution.md` with the default template, erasing any customizations you made.

**Workaround:**

```bash
# 1. Back up your constitution before upgrading
cp .pmf/memory/constitution.md .pmf/memory/constitution-backup.md

# 2. Run the upgrade
specify init --here --force --ai copilot

# 3. Restore your customized constitution
mv .pmf/memory/constitution-backup.md .pmf/memory/constitution.md
```
...
git restore .pmf/memory/constitution.md
```

### 2. Custom template modifications

If you customized any templates in `.pmf/templates/`, the upgrade will overwrite them. Back them up first:

```bash
# Back up custom templates
cp -r .pmf/templates .pmf/templates-backup
...
rm pmfkit.specify-old.md
rm pmfkit.plan-v1.md
```
...
cp .pmf/memory/constitution.md /tmp/constitution-backup.md
cp -r .pmf/templates /tmp/templates-backup
...
mv /tmp/constitution-backup.md .pmf/memory/constitution.md
...
cp /tmp/constitution-backup.md .pmf/memory/constitution.md
```
...
3. **Some files may be overwritten** - If you already have Spec Kit files (`.claude/`, `.pmf/`, etc.), they'll be replaced with the new versions

**What gets overwritten:**

Only Spec Kit infrastructure files:
- Agent command files (`.claude/commands/`, `.github/prompts/`, etc.)
- Scripts in `.pmf/scripts/`
- Templates in `.pmf/templates/`
- Memory files in `.pmf/memory/` (including constitution)
...
**Prevention tip:** Before upgrading, commit or back up your `.pmf/memory/constitution.md` if you customized it.
...
Once you've run `specify init`, the slash commands (like `/pmfkit.pmf`, `/pmfkit.plan`, etc.) are **permanently installed** in your project's agent folder (`.claude/`, `.github/prompts/`, etc.). Your AI assistant reads these command files directly—no need to run `specify` again.
...
- **Test new slash commands:** Run `/pmfkit.constitution` or another command to verify everything works
