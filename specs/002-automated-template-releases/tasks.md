...
### 📝 Task 1.2: Update Command Templates for PMF-Kit Namespace
...
**Description**:
Update all command template files in `templates/commands/` to ensure they use PMF-Kit branding and `/pmfkit.*` namespace instead of spec-kit references.

**Implementation Steps**:
1. Review each `.md` file in `templates/commands/` directory
2. For each file:
   - Verify frontmatter uses `agent: __AGENT__` placeholder
   - Check body text for `/speckit.` references → replace with `/pmfkit.`
   - Verify path references use .specify/ prefix
   - Confirm `scripts.sh` and `scripts.ps` point to correct bash/powershell directories
   - Ensure descriptions mention PMF-Kit (not Spec-Kit)
...
- ✅ Descriptions mention "PMF-Kit" not "Spec-Kit"
...
7. Verify constitution is PMF-Kit v1.0.0
8. Check command file frontmatter uses `pmfkit.*` namespace
...
head -5 /tmp/test-claude/.claude/commands/pmfkit.specify.md
# Should show: agent: pmfkit.specify

# 7. Count command files
ls /tmp/test-claude/.claude/commands/pmfkit.*.md | wc -l
# Should output: 9

# 8. Verify constitution
grep "PMF-Kit Constitution" /tmp/test-claude/.specify/memory/constitution.md
...
- ✅ Command files use `pmfkit.*` naming and namespace
- ✅ Constitution is PMF-Kit v1.0.0 (not Spec-Kit)
...
ls /tmp/spot-claude/.claude/commands/pmfkit.*.md | wc -l
# Should output: 9

# 6. Spot-check Gemini
unzip -q -d /tmp/spot-gemini dist/templates/spec-kit-template-gemini-ps-v0.1.0-test.zip
ls /tmp/spot-gemini/.gemini/commands/pmfkit.*.toml | wc -l
# Should output: 9
grep "{{args}}" /tmp/spot-gemini/.gemini/commands/pmfkit.specify.toml
...
ls /tmp/spot-copilot/.github/agents/pmfkit.*.agent.md | wc -l
# Should output: 9
...
2. Implement validation functions:
   - `check_frontmatter_namespace()`: Verify all commands use `agent: pmfkit.*`
   ...
   - `check_constitution_version()`: Verify PMF-Kit constitution v1.0.0
...
check_frontmatter_namespace() {
  ...
  find "$extract_dir/$agent_dir" -name "pmfkit.*.md" -o -name "pmfkit.*.toml" -o -name "pmfkit.*.agent.md" | while read -r file; do
...
sed -i 's/agent: pmfkit\.specify/agent: speckit.specify/' test-data/wrong-namespace/.claude/commands/pmfkit.specify.md
...
echo "Run /speckit.plan to continue" >> test-data/speckit-ref/.claude/commands/pmfkit.specify.md
...
### 📄 Task 3.2: Update Release Notes Script for PMF-Kit Branding
...
**Description**:
Update the release notes generation script to reference "PMF CLI" instead of "Specify CLI" and include SHA-256 checksums for all template archives.

**Implementation Steps**:
1. Open `.github/workflows/scripts/generate-release-notes.sh`
2. Update intro text to say "PMF CLI" not "Specify CLI"
...
**Acceptance Criteria**:
- ✅ Release notes mention "PMF CLI" (not "Specify CLI")
...
# Go to: https://github.com/YOUR_ORG/pmf-kit/actions/workflows/release.yml
...
### 🚀 Task 4.1: Create First PMF-Kit Release (v0.1.0)
...
**Description**:
Merge the feature branch to main and trigger the automated release workflow to publish the first official PMF-Kit templates release.
...
6. Monitor workflow at: https://github.com/YOUR_ORG/pmf-kit/actions
7. Wait for workflow to complete (~5-10 minutes)
8. Verify release at: https://github.com/YOUR_ORG/pmf-kit/releases
...
# Go to: https://github.com/YOUR_ORG/pmf-kit/actions
...
- ✅ Release v0.1.0 appears at: https://github.com/YOUR_ORG/pmf-kit/releases/latest
...
### ✅ Task 4.2: Verify CLI Downloads PMF-Kit Templates
...
**Description**:
Verify that the PMF CLI successfully downloads templates from the pmf-kit repository (not falling back to spec-kit) and that templates work correctly when extracted.

**Implementation Steps**:
1. Install PMF CLI from source (or pypi if published)
...
5. Check command files use pmfkit.* namespace
6. Verify constitution is PMF-Kit v1.0.0
...
pmf init test-project-claude --agent claude --script sh
...
# 4. Check namespace
head -10 test-project-claude/.claude/commands/pmfkit.specify.md
# Should show: agent: pmfkit.specify

# 5. Verify constitution
grep "PMF-Kit Constitution" test-project-claude/.specify/memory/constitution.md
...
pmf init test-project-gemini --agent gemini --script ps

# 8. Verify Gemini structure
ls test-project-gemini/.gemini/commands/pmfkit.*.toml
grep "{{args}}" test-project-gemini/.gemini/commands/pmfkit.specify.toml

# 9. Check CLI logs to confirm source
# Look for: "Downloading from pmf-kit releases..."
# NOT: "Falling back to spec-kit releases..."

# 10. Manual test: Open AI agent and run /pmfkit.specify
# (This requires actual AI agent testing - beyond automated scope)
```

**Acceptance Criteria**:
- ✅ CLI successfully downloads templates from pmf-kit repository
- ✅ No fallback to spec-kit occurs (check CLI logs)
- ✅ Extracted templates have correct directory structure
- ✅ Command files use pmfkit.* namespace (not speckit.*)
- ✅ Constitution is PMF-Kit v1.0.0 (not Spec-Kit)
...
**Description**:
Remove the fallback logic that downloads templates from spec-kit when pmf-kit templates aren't found, since PMF-Kit now has its own releases.

**Implementation Steps**:
1. Open `src/pmf_cli/__init__.py`
2. Find `download_template_from_github()` function
3. Remove try/except logic that falls back to spec-kit
4. Keep only direct download from pmf-kit
5. Update error messages to guide user to check pmf-kit releases
6. Update tests to reflect new behavior

**Code Changes**:
```python
# In src/pmf_cli/__init__.py

def download_template_from_github(agent: str, script: str, version: str) -> Path:
    """Download template from PMF-Kit releases.
    ...
    release_url = (
        f"https://github.com/YOUR_ORG/pmf-kit/releases/download/{version}/{template_name}"
    )

    # Download from pmf-kit releases (no fallback)
    try:
        ...
        if e.response.status_code == 404:
            raise ValueError(
                ...
                f"Check available releases at: https://github.com/YOUR_ORG/pmf-kit/releases"
            ) from e
        raise
...
**Files to Modify**:
- `src/pmf_cli/__init__.py` (remove ~20 lines, update ~10 lines)
- `tests/test_cli.py` (update tests to not expect fallback)

**Testing**:
```bash
# Test with valid version
pmf init test-valid --agent claude --script sh --version v0.1.0
# Should succeed

# Test with invalid version
pmf init test-invalid --agent claude --script sh --version v99.99.99
# Should fail with clear error message (not fallback)

# Test with invalid agent
pmf init test-invalid-agent --agent nonexistent --script sh
...
- ✅ CLI only attempts to download from pmf-kit repository
- ✅ Error message clearly indicates to check pmf-kit releases
...
**Description**:
Update project documentation to explain PMF-Kit templates, release process, and usage.
...
PMF-Kit provides project templates for 18 AI coding agents, available in both bash and PowerShell variants.
...
Templates are automatically downloaded when you run `pmf init`.

### Manual Download

You can also download templates directly from [GitHub Releases](https://github.com/YOUR_ORG/pmf-kit/releases/latest) if you prefer manual installation.
```
...
PMF-Kit templates provide a structured starting point for product-market-fit discovery projects using AI coding agents.
...
|---------|-------------|-------|
| /pmfkit.specify | Create or update feature specification | `/pmfkit.specify "Add user authentication"` |
| /pmfkit.plan | Generate implementation plan | `/pmfkit.plan` |
| /pmfkit.tasks | Break plan into actionable tasks | `/pmfkit.tasks` |
| /pmfkit.implement | Execute tasks systematically | `/pmfkit.implement` |
| /pmfkit.clarify | Identify underspecified areas | `/pmfkit.clarify` |
| /pmfkit.analyze | Cross-artifact consistency check | `/pmfkit.analyze` |
| /pmfkit.checklist | Generate custom checklist | `/pmfkit.checklist` |
| /pmfkit.taskstoissues | Convert tasks to GitHub issues | `/pmfkit.taskstoissues` |
| /pmfkit.constitution | Manage project principles | `/pmfkit.constitution` |
...
See [GitHub Releases](https://github.com/YOUR_ORG/pmf-kit/releases) for full release history and changelogs.
```
...
# Test with CLI
pmf init test-project --template file://dist/templates/spec-kit-template-claude-sh-v0.X.Y-test.zip
```
...
Watch workflow progress at: https://github.com/YOUR_ORG/pmf-kit/actions
...
**Acceptance Criteria**:
- ✅ README mentions PMF-Kit templates (not spec-kit)
...
5. Test with CLI: `pmf init test --template file://...`
6. Create PR
